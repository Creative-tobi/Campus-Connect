...
Campus Connect Team`;
        ...
        res.status(201).json({
            message: 'Club created successfully and awaiting admin approval. Your role has been updated to club owner.',
            club: newClub
        });
...
const getMyClubs = async (req, res) => {
    try {
        ...
        const clubs = await Club.find({ owner: userId }).select('name description category status memberCount logo banner createdAt');
        res.json({ clubs });
...
const updateClub = async (req, res) => {
    try {
        ...
        if (!club) {
            return res.status(404).json({ error: 'Club not found or you are not the owner' });
        }

        // Prevent updating name if it conflicts with another club (except itself)
        if (name && name.trim() !== club.name) {
            const existingClub = await Club.findOne({ name: name.trim(), _id: { $ne: clubId } });
            if (existingClub) {
                return res.status(400).json({ error: 'A club with this name already exists' });
            }
            club.name = name.trim();
...
const getClubById = async (req, res) => {
    try {
        ...
        if (club.status !== 'active' && club.owner._id.toString() !== req.user.id && req.user.role !== 'admin') {
...
const getJoinRequests = async (req, res) => {
    try {
        ...
        const club = await Club.findById(clubId).populate('members.user', 'firstName lastName email profilePicture');
        if (!club || club.owner.toString() !== userId) {
            return res.status(403).json({ error: 'Access denied. You are not the club owner.' });
        }
...
const respondToJoinRequest = async (req, res) => {
    try {
        ...
        if (action === 'approve') {
            ...
            const isMember = club.members.some(m => m.user.toString() === userIdToRespond);
            if (isMember) {
...
Campus Connect Team`;
        ...
        res.json({ message: `Join request ${action === 'approve' ? 'approved' : 'declined'} successfully` });
...
const createPost = async (req, res) => {
    try {
        ...
        for (const memberObj of club.members) {
            if (memberObj.user.toString() !== userId) { // Don't notify the author
                const notification = new Notification({
...
const deletePost = async (req, res) => {
    try {
        ...
        if (post.author.toString() !== userId || post.club.owner.toString() !== userId) {
            return res.status(403).json({ error: 'Access denied. You are not authorized to delete this post.' });
        }
...
module.exports = {
    ...
    getClubPosts,
    deletePost,
    uploadClubImageMiddleware: upload.fields([{ name: 'logo', maxCount: 1 }, { name: 'banner', maxCount: 1 }, { name: 'postMedia', maxCount: 1 }]) // Export middleware
};